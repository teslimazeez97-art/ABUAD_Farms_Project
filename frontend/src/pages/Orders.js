...
const Orders = () => {
  ...
  const getStoredToken = () => {
    // Try common storage shapes: plain token or user object with token
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("⚠️ [DEBUG] Orders response not JSON:", text);
        data = {};
      }
      console.log("📦 [DEBUG] Raw orders response:", data);

      // tolerant detection of the orders list
      let list = [];
      if (Array.isArray(data)) {
        ...
        list = data.data;
      } else if (data && data.success === true && Array.isArray(data.orders)) {
        list = data.orders;
      } else {
        console.warn("⚠️ [DEBUG] No orders array detected. Keys:", Object.keys(data || {}));
        list = [];
      }

      // normalize each order
      const processed = list.map((order) => {
        const items = (order.items || order.items_list || order.order_items || []).map((it) => ({
          ...it,
        ...
        return {
          id: order.id || order.order_id || order._id || null,
          order_number: order.order_number || order.reference || null,
          ...
          items,
          total,
          item_count: items.length || toNum(order.item_count || order.items_count || 0),
          ...order,
        };
      });

      console.log("ℹ️ [DEBUG] Parsed orders count:", processed.length, "sample:", processed[0] || null);
      setOrders(processed);
    } catch (err) {
      console.error("Error fetching orders:", err);
      setError(err.message || "Failed to fetch orders");
      setOrders([]); // ensure state is consistent
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    console.log("STATE: orders length =", orders.length);
  }, [orders]);
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesSearch =
      ...
      (order.customer_email || order.email || "").toLowerCase().includes(q);
              ...
              <tbody>
                {filteredOrders.length > 0 ? (
                  filteredOrders.map((order, index) => (
                    <tr key={order.id ?? index} style={{ backgroundColor: index % 2 === 0 ? "white" : "#f9fafb", borderTop: "1px solid #f3f4f6" }}>
