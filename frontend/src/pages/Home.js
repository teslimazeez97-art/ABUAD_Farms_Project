...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        console.log("🏷️ Home: Fetching featured products from:", `${API}/api/products/featured`);
        ...
        console.log("? Home: Featured products response:", featRes.status, Array.isArray(featRes.data) ? featRes.data.length : typeof featRes.data);
        console.log("? Home: All products response:", prodRes.status, Array.isArray(prodRes.data) ? prodRes.data.length : typeof prodRes.data);
        if (Array.isArray(featRes.data) && featRes.data.length > 0) {
          console.log("ℹ️ Home: Sample featured product:", featRes.data[0]);
        }
  ...
  // Derive categories only after products are loaded
  ...
  // Log only once after categories are derived
  useEffect(() => {
    ...
    <div>
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: 40 }}>
          <div style={{ fontSize: 30, marginBottom: 10 }}><FaSpinner className="spin" /></div>
              ...
              featured.map(p => (
                ...
                >
                  ...
                  <div style={{ padding: 12 }}>
                    ...
                    <div style={{ color: "#2f855a", fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 14 }}>₦{Number(p.price).toLocaleString()}</span>
                      {p.featured && <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><FaStar color="#fbbf24" /></span>}
                    </div>
                  </div>
                </div>
              ))
